...
AddressModelHandler::AddressModelHandler(QObject* _parent)
    : QObject{_parent}, m_selectedItemsCount{}
{
}
...
void AddressModelHandler::setItemSelected(int _itemIndex, int _itemValue)
{
    ...
    auto firstEmpty = std::ranges::find_if(
        m_addressRecords, [](const auto& _item) { return !_item.component.has_value(); });
...
void AddressModelHandler::setItemDeselected(int _itemIndex)
{
    ...
    auto requiredNode = std::ranges::find_if(m_addressRecords, [_itemIndex](const auto& _item) {
        return _item.itemIndex == _itemIndex;
    });
...
void AddressModelHandler::onItemEditFinished(QString _itemValue, int _itemIndex)
{
    if (_itemValue.isEmpty())
    {
        ...
        {
            if (const auto& itemIndexOpt = m_addressRecords.at(_itemIndex).itemIndex;
                itemIndexOpt.has_value())
            {
...
{
    std::ranges::for_each(m_addressRecords, [this](const auto& item) {
...
bool AddressModelHandler::containsValue(int _itemIndex) const
{
    ...
    auto itemIt = std::ranges::find_if(m_addressRecords, [_itemIndex](const auto& _item) {
...
QString AddressModelHandler::getAvailableAddressParts()const
{
    if(isAddressValid())
        return getCurrentAddress();

    std::string avaiableParts;

    for(const auto& addressPart :  m_addressRecords)
    {
        if( addressPart.component.has_value())
        {
            avaiableParts+= std::to_string(addressPart.component.value());
            avaiableParts +=':';
        }
    };

    return QString::fromStdString(avaiableParts);
}

void AddressModelHandler::registerQmlType()
{
    qmlRegisterType<AddressModelHandler>("app.address_model", 1, 0, "AddressModelHandler");
}
